...
await new Promise(resolve => setTimeout(resolve, 1000));
  ...
  // Automatically create the Fruity Socials panel.
  try {
    ...
    if (socialsResult.action === 'created') {
      startupLog(`✅ Fruity Socials panel created (${socialsResult.messageId})`);
    } else if (socialsResult.action === 'unchanged') {
      startupLog(`✅ Fruity Socials panel already exists (${socialsResult.messageId})`);
    } else {
      logger.error(
        `❌ Failed to reconcile Fruity Socials panel: ${socialsResult.error || 'Unknown error'}`
      );
    }
  } catch (error) {
    logger.error('❌ Failed to create/update Fruity Socials panel:', error);
  }
