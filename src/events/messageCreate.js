...
async function handleLeveling(
  ...
  try {
    ...
    const cooldownTime =
      Number.isFinite(
        Number(levelingConfig.xpCooldown)
      )
        ? Number(
    ...
    if (
      Number(levelingConfig.xpMultiplier) > 1
    ) {
      finalXP =
        Math.floor(
          finalXP *
    ...
    logger.debug(
      `Awarded ${finalXP} XP to ${message.author.tag} in ${message.guild.name}`
    );
  } catch (error) {
    logger.error(
      `Error handling leveling for ${message.author.tag}:`,
      error
    );
  }
}
